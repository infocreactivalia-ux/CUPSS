...
export default function RegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    const { data, error } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { rol: 'hostelero', nombre } },
    })
    ...
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#0a0a0f',padding:'20px'}}>
      <div style={{width:'100%',maxWidth:'420px'}}>
        <div style={{textAlign:'center',marginBottom:'40px'}}>
          ...
          <div style={{fontSize:'12px',color:'#5a5a72',letterSpacing:'3px',marginTop:'4px'}}>CREAR CUENTA</div>
        </div>
        <div style={{background:'#111118',border:'1px solid rgba(255,255,255,0.07)',borderRadius:'16px',padding:'32px'}}>
          <h1 style={{fontSize:'18px',fontWeight:'700',marginBottom:'24px',color:'#f0f0f5'}}>Registro gratuito</h1>
          ...
          <form onSubmit={handleRegister}>
            <div style={{marginBottom:'14px'}}>
              <label style={{display:'block',fontSize:'11px',color:'#9090a8',textTransform:'uppercase',letterSpacing:'1px',marginBottom:'6px'}}>Tu nombre</label>
            ...
            <div style={{marginBottom:'14px'}}>
              <label style={{display:'block',fontSize:'11px',color:'#9090a8',textTransform:'uppercase',letterSpacing:'1px',marginBottom:'6px'}}>Nombre del local</label>
              <input type="text" value={nombreLocal} onChange={e=>setNombreLocal(e.target.value)} required placeholder="Restaurante El Rincón"
                style={{width:'100%',background:'#18181f',border:'1px solid rgba(255,255,255,0.11)',borderRadius:'8px',padding:'10px 14px',color:'#f0f0f5',fontSize:'13px',outline:'none',boxSizing:'border-box'}} />
            </div>
            <div style={{marginBottom:'14px'}}>
          ...
          <div style={{marginTop:'24px',paddingTop:'24px',borderTop:'1px solid rgba(255,255,255,0.07)',textAlign:'center',fontSize:'13px',color:'#9090a8'}}>
            ¿Ya tienes cuenta? <Link href="/auth/login" style={{color:'#f0c14b'}}>Acceder</Link>
          </div>
        </div>
      </div>
    </div>
